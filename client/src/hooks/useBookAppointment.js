...
export const useBookAppointment = () => {
  ...
  const bookAppointment = async (doctorId, patientId, date, time) => {
    setIsLoading(true);
    setError(null);

    try {
      // Make an API call to book the appointment
      const response = await axios.post('http://localhost:5000/appointment/create', { doctorId, patientId, date, time });
